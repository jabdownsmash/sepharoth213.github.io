...
class Primitives
{
    ...
    public static function drawRect(?drawTarget:GraphicObject, x0:Float, y0:Float, width:Float, height:Float, color:ColorObject, antiAlias:Bool=false)
    {
        if(drawTarget == null)
        {
        ...
        // for(i in 0...m.floor(width))
        // {
        //     drawSolidLine(drawTarget, x+i, y, x+i, y+height, color);
        // }
        for(x in m.floor(x0)...m.floor(x0+width))
        {
            for(y in m.floor(y0)...m.floor(y0+height))
            {
    ...
    public static function drawCircle(?drawTarget:GraphicObject, centerX:Float, centerY:Float, radius:Float, color:ColorObject, antiAlias:Bool=false, lineWidth:Float=1)
    {
        ...
        for(x in 0...m.floor(c))
        {
            if(lineWidth==0)
            {
                ...
                drawLine(drawTarget, m.round(centerX+x), m.round(centerY), m.round(centerX + x), m.round(centerY+y), color, lineWidth, antiAlias);
                drawLine(drawTarget, m.round(centerX+x), m.round(centerY), m.round(centerX + x), m.round(centerY-y), color, lineWidth, antiAlias);
                drawLine(drawTarget, m.round(centerX-x), m.round(centerY), m.round(centerX - x), m.round(centerY+y), color, lineWidth, antiAlias);
                drawLine(drawTarget, m.round(centerX-x), m.round(centerY), m.round(centerX - x), m.round(centerY-y), color, lineWidth, antiAlias);
                drawLine(drawTarget, m.round(centerX+y), m.round(centerY), m.round(centerX + y), m.round(centerY+x), color, lineWidth, antiAlias);
                drawLine(drawTarget, m.round(centerX+y), m.round(centerY), m.round(centerX + y), m.round(centerY-x), color, lineWidth, antiAlias);
                drawLine(drawTarget, m.round(centerX-y), m.round(centerY), m.round(centerX - y), m.round(centerY+x), color, lineWidth, antiAlias);
                drawLine(drawTarget, m.round(centerX-y), m.round(centerY), m.round(centerX - y), m.round(centerY-x), color, lineWidth, antiAlias);
            }
    ...
    public static function drawPolygon(?drawTarget:GraphicObject, points:Array<Array<Float>>, color:ColorObject, lineWidth:Int=1, antiAlias:Bool=false)
    {
        ...
        if(points.length > 1)
        {
            Primitives.drawSolidLine(drawTarget, points[0][0],points[0][1],points[1][0],points[1][1],new ColorObject(1,1,1));
        }
