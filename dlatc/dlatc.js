...
ApplicationMain.embed = $hx_exports.openfl.embed = function(elementName,width,height,background) {
	...
	ApplicationMain.urlLoaders.set("assets/lvl2-add.xmp",urlLoader);
	ApplicationMain.total++;
	var urlLoader1 = new openfl.net.URLLoader();
	urlLoader1.set_dataFormat(openfl.net.URLLoaderDataFormat.BINARY);
	ApplicationMain.urlLoaders.set("assets/lvl3-base.xmp",urlLoader1);
	ApplicationMain.total++;
	var sound = haxe.io.Path.withoutExtension("assets/placeholder.ogg");
	if(!HxOverrides.remove(sounds,sound)) ApplicationMain.total++;
	sounds.push(sound);
	var sound1 = haxe.io.Path.withoutExtension("assets/snare.ogg");
	if(!HxOverrides.remove(sounds,sound1)) ApplicationMain.total++;
	sounds.push(sound1);
	var sound2 = haxe.io.Path.withoutExtension("assets/lvl1-base.ogg");
	if(!HxOverrides.remove(sounds,sound2)) ApplicationMain.total++;
	sounds.push(sound2);
	var sound3 = haxe.io.Path.withoutExtension("assets/lvl2-add.ogg");
	...
	image.src = id;
	ApplicationMain.total++;
	var sound4 = haxe.io.Path.withoutExtension("assets/lvl3-add.ogg");
	if(!HxOverrides.remove(sounds,sound4)) ApplicationMain.total++;
	sounds.push(sound4);
	var sound5 = haxe.io.Path.withoutExtension("assets/hat.ogg");
	...
	ApplicationMain.urlLoaders.set("assets/lvl1-add.xmp",urlLoader2);
	ApplicationMain.total++;
	var sound6 = haxe.io.Path.withoutExtension("assets/fart.ogg");
	...
	ApplicationMain.urlLoaders.set("assets/songlist.json",urlLoader3);
	ApplicationMain.total++;
	var sound7 = haxe.io.Path.withoutExtension("assets/lvl3-base.ogg");
	...
	ApplicationMain.urlLoaders.set("assets/lvl1-base.xmp",urlLoader4);
	ApplicationMain.total++;
	var urlLoader5 = new openfl.net.URLLoader();
	urlLoader5.set_dataFormat(openfl.net.URLLoaderDataFormat.BINARY);
	ApplicationMain.urlLoaders.set("assets/creditsong.xmp",urlLoader5);
	ApplicationMain.total++;
	var sound8 = haxe.io.Path.withoutExtension("assets/lvl2-base.ogg");
	...
	ApplicationMain.urlLoaders.set("assets/lvl2-base.xmp",urlLoader6);
	ApplicationMain.total++;
	var sound9 = haxe.io.Path.withoutExtension("assets/overworld-base.ogg");
	if(!HxOverrides.remove(sounds,sound9)) ApplicationMain.total++;
	sounds.push(sound9);
	var sound10 = haxe.io.Path.withoutExtension("assets/creditsong.ogg");
	...
	ApplicationMain.urlLoaders.set("assets/overworld-base.xmp",urlLoader7);
	ApplicationMain.total++;
	var sound11 = haxe.io.Path.withoutExtension("assets/dickbutt.ogg");
	...
	ApplicationMain.urlLoaders.set("assets/song.json",urlLoader8);
	ApplicationMain.total++;
	var sound12 = haxe.io.Path.withoutExtension("assets/lvl1-add.ogg");
	...
	ApplicationMain.urlLoaders.set("assets/lvl3-add.xmp",urlLoader9);
	ApplicationMain.total++;
	if(ApplicationMain.total == 0) ApplicationMain.start(); else {
		...
		while(_g < sounds.length) {
			...
			++_g;
			var sound13 = new openfl.media.Sound();
			sound13.addEventListener(openfl.events.Event.COMPLETE,ApplicationMain.sound_onComplete);
			sound13.addEventListener(openfl.events.IOErrorEvent.IO_ERROR,ApplicationMain.sound_onIOError);
			sound13.load(new openfl.net.URLRequest(soundName + ".ogg"));
		}
	}
};
...
ApplicationMain.preloader_onComplete = function(event) {
	...
	if(hasMain) Reflect.callMethod(Main,Reflect.field(Main,"main"),[]); else {
		...
		if(js.Boot.__instanceof(instance,openfl.display.DisplayObject)) openfl.Lib.current.addChild(instance); else {
			haxe.Log.trace("Error: No entry point found",{ fileName : "ApplicationMain.hx", lineNumber : 376, className : "ApplicationMain", methodName : "preloader_onComplete"});
			haxe.Log.trace("If you are using DCE with a static main, you may need to @:keep the function",{ fileName : "ApplicationMain.hx", lineNumber : 377, className : "ApplicationMain", methodName : "preloader_onComplete"});
		}
	}
};
...
var Main = function() {
	this.mazeWorld = false;
	this.avoidWorld = false;
	this.collectWorld = false;
	...
	this.viewport = new openfl.display.Sprite();
...
Main.prototype = $extend(haxel.Core.prototype,{
	loadTitleScreen: function() {
		haxel.Time.paused = true;
		this.init();
		var level = new TitleScreen(this);
		this.drawFrame();
		haxel.Time.paused = false;
	}
	,loadOverworld: function() {
		haxel.Time.paused = true;
		this.init();
		if(this.collectWorld && this.avoidWorld && this.mazeWorld) {
			var level = new EndScreen(this);
		} else {
			var level1 = new Overworld(this);
		}
		this.drawFrame();
		haxel.Time.paused = false;
	}
	,loadCollectWorld: function() {
		haxel.Time.paused = true;
		this.init();
	...
	,init: function() {
		...
		background.get_graphics().drawRect(0,0,this.screenWidth,this.screenHeight);
		background.get_graphics().endFill();
		this.addChild(background);
	}
	,drawFrame: function() {
		this.set_x(this.stage.stageWidth / 2 - this.screenWidth / 2);
		this.set_y(this.stage.stageHeight / 2 - this.screenHeight / 2);
		var frame = new openfl.display.Sprite();
		frame.get_graphics().beginFill(394773);
		frame.get_graphics().drawRect(0,0,this.get_x(),this.stage.stageHeight);
		frame.get_graphics().drawRect(this.get_x() + this.screenWidth,0,this.get_x(),this.stage.stageHeight);
		frame.get_graphics().drawRect(this.get_x(),0,this.screenWidth,this.get_y());
		frame.get_graphics().drawRect(this.get_x(),this.get_y() + this.screenHeight - 1,this.screenWidth,this.get_y() + 30);
		frame.get_graphics().endFill();
		frame.set_x(-this.get_x());
		frame.set_y(-this.get_y());
		this.addChild(frame);
	}
	,removeAll: function() {
		while(this.get_numChildren() > 0) this.removeChildAt(0);
...
var World = function(main) {
	...
	this.instance = main;
	this.playing = false;
	this.songObjectXOffset = 0;
	this.songObjects = new Array();
	this.objectBaseSpeed = this.columnY / (1 / this.bpm * 60 * 1000) / 4;
	this.lastReportedPlayheadPosition = 0;
	this.currentSongTime = 0;
	this.addChild(this.instance.viewport);
	SongObject.viewport = this.instance.viewport;
	this.player = new openfl.display.Sprite();
	this.player.get_graphics().beginFill(14544605);
	this.player.get_graphics().drawCircle(0,0,this.playerSize);
	this.player.get_graphics().endFill();
	this.player.set_x(this.instance.screenWidth / 2);
	this.player.set_y(this.columnY);
	this.addChild(this.player);
	this.playerCollectable = new openfl.display.Sprite();
	this.playerCollectable.get_graphics().beginFill(1170722);
	this.playerCollectable.get_graphics().drawCircle(0,0,12);
	this.playerCollectable.get_graphics().endFill();
	this.playerCollectable.set_x(60);
	this.collectableContainer = new openfl.display.Sprite();
	this.collectableContainer.addChild(this.playerCollectable);
	this.collectableContainer.set_x(this.instance.screenWidth / 2);
	this.collectableContainer.set_y(this.columnY);
...
World.prototype = {
	...
	,check: function(songObject) {
		var dx = this.player.get_x() - songObject.get_x();
		var dy = this.player.get_y() - songObject.get_y();
		var distance = Math.sqrt(dx * dx + dy * dy);
		return distance < this.playerSize + songObject.size;
	}
	,start: function() {
		this.soundChannel = this.song.play(0,-1);
		haxel.Time.callbackFunction = $bind(this,this.update);
	}
	,spawnObject: function(size,sprite,beatNumber,x) {
		var newObject = new SongObject(sprite,size,-beatNumber * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4 + 100,x,this.instance.screenWidth,this.loopBeats * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4,this.objectBaseSpeed);
		this.songObjects.push(newObject);
		this.addChild(newObject);
		return newObject;
	}
	,createSprite: function(color,radius,yRespawnDistance) {
		var sprite = new openfl.display.Sprite();
		sprite.get_graphics().beginFill(color);
		sprite.get_graphics().drawCircle(radius,radius,radius);
		sprite.get_graphics().endFill();
		var bitmapData = new openfl.display.BitmapData(radius * 2 | 0,radius * 2 | 0,true,0);
		bitmapData.draw(sprite);
		var sprites = new openfl.display.Tilesheet(bitmapData);
		sprites.addTileRect(new openfl.geom.Rectangle(0,0,radius * 2 | 0,radius * 2 | 0));
		return sprites;
	}
	,update: function(deltaTime) {
		SongObject.viewport.get_graphics().clear();
		this.currentSongTime += deltaTime;
		if(this.soundChannel.get_position() != this.lastReportedPlayheadPosition) {
			if(this.soundChannel.get_position() < this.lastReportedPlayheadPosition) {
				this.currentSongTime = this.soundChannel.get_position();
		...
		if(haxel.KeyboardInput.check(37)) {
			this.playerVelocity += this.playerAcceleration;
			playerNotMoving = false;
		}
		if(haxel.KeyboardInput.check(39)) {
			playerNotMoving = false;
			this.playerVelocity -= this.playerAcceleration;
		}
		if(haxel.MouseInput.mouseDown) {
			if(haxel.MouseInput.get_mouseX() > this.instance.screenWidth / 2) {
				playerNotMoving = false;
				this.playerVelocity += this.playerAcceleration;
			} else {
				playerNotMoving = false;
		...
		if(this.playerVelocity > this.playerMaxSpeed) this.playerVelocity = this.playerMaxSpeed;
		if(this.playerVelocity < -this.playerMaxSpeed) this.playerVelocity = -this.playerMaxSpeed;
		this.songObjectXOffset += this.playerVelocity;
		if(playerNotMoving) this.playerVelocity = this.playerVelocity * this.friction;
		var _g = 0;
		var _g1 = this.songObjects;
		while(_g < _g1.length) {
			var songObject = _g1[_g];
			++_g;
			songObject.update(this.currentSongTime,this.songObjectXOffset);
		}
	}
	,__class__: World
};
var AvoidWorld = function(main) {
	this.columnStart = 50;
	this.bpm = 112;
	this.maxColumns = 5;
	this.song = openfl.Assets.getSound("assets/lvl2-base.ogg");
	this.song2 = openfl.Assets.getSound("assets/lvl2-add.ogg");
	this.loopBeats = 128;
	World.call(this,main);
	this.overworldTeleporterSprite = this.createSprite(14540253,30,this.loopBeats * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4);
	this.overworldTeleporter = this.spawnObject(30,this.overworldTeleporterSprite,113,this.instance.screenWidth / 2 + 100);
	this.collectableSprite = this.createSprite(1170722,12,this.loopBeats * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4);
	this.obstacleSprite = this.createSprite(14492177,20,this.loopBeats * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4);
	this.collectable = this.spawnObject(12,this.collectableSprite,6,200);
	this.obstacles = new Array();
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,30,240));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,32,180));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,33,40));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,33,80));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,34,340));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,35,240));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,36,300));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,38,110));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,41,90));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,42,120));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,44,380));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,44,200));
	this.start();
};
$hxClasses["AvoidWorld"] = AvoidWorld;
AvoidWorld.__name__ = ["AvoidWorld"];
AvoidWorld.__super__ = World;
AvoidWorld.prototype = $extend(World.prototype,{
	drawBackground: function(numColumns,lineWidth) {
		this.columnWidth = this.instance.screenWidth / this.maxColumns;
		this.lineBackground = new openfl.display.Sprite();
		this.lineBackground.get_graphics().beginFill(328967);
		var _g1 = -numColumns;
		var _g = numColumns * 2;
		while(_g1 < _g) {
			var i = _g1++;
			this.lineBackground.get_graphics().drawRect(this.columnStart + i * this.columnWidth - lineWidth / 2,0,lineWidth,this.instance.screenHeight);
		}
		this.lineBackground.get_graphics().endFill();
	}
	,update: function(deltaTime) {
		World.prototype.update.call(this,deltaTime);
		var _g = this.collectableContainer;
		_g.set_rotation(_g.get_rotation() + 3);
		if(this.check(this.collectable)) this.hasCollectable = true;
		var _g1 = 0;
		var _g11 = this.obstacles;
		while(_g1 < _g11.length) {
			var obstacle = _g11[_g1];
			++_g1;
			if(this.check(obstacle)) this.hasCollectable = false;
		}
		if(this.hasCollectable) {
			this.collectable.set_visible(false);
			this.playerCollectable.set_visible(true);
			if(!this.playing) {
				this.soundChannel2 = this.song2.play(this.soundChannel.get_position(),-1);
				this.playing = true;
			}
		} else {
			this.collectable.set_visible(true);
			this.playerCollectable.set_visible(false);
			if(this.soundChannel2 != null) this.soundChannel2.stop();
			this.playing = false;
		}
		if(this.check(this.overworldTeleporter) && this.hasCollectable) {
			this.soundChannel.stop();
			this.soundChannel2.stop();
			this.instance.avoidWorld = true;
			this.instance.loadOverworld();
		}
	}
	,__class__: AvoidWorld
});
var CollectWorld = function(main) {
	this.columnStart = 50;
	this.bpm = 112;
	this.maxColumns = 5;
	this.song = openfl.Assets.getSound("assets/lvl1-base.ogg");
	this.song2 = openfl.Assets.getSound("assets/lvl1-add.ogg");
	this.loopBeats = 128;
	World.call(this,main);
	this.drawBackground(this.maxColumns,4);
	this.overworldTeleporterSprite = this.createSprite(14540253,30,this.loopBeats * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4);
	this.overworldTeleporter = this.spawnObject(30,this.overworldTeleporterSprite,113,this.instance.screenWidth / 2 + 100);
	this.collectableSprite = this.createSprite(1170722,12,this.loopBeats * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4);
	this.obstacleSprite = this.createSprite(14492177,20,this.loopBeats * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4);
	this.collectables = new Array();
	this.collectables.push(this.spawnObject(12,this.collectableSprite,10,150));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,20,320));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,20,460));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,26,220));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,26,150));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,34,240));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,38,350));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,40,120));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,40,110));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,46,320));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,50,350));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,56,360));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,62,210));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,66,220));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,76,250));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,82,230));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,90,350));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,90,360));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,96,210));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,96,220));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,96,250));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,100,260));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,104,360));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,106,210));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,106,160));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,109,250));
	this.collectables.push(this.spawnObject(12,this.collectableSprite,109,360));
	this.resetCollectables();
	this.obstacles = new Array();
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,11,40));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,14,280));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,18,140));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,20,40));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,23,20));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,23,140));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,23,300));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,49,110));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,70,90));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,15,120));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,15,380));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,15,200));
	this.start();
};
$hxClasses["CollectWorld"] = CollectWorld;
CollectWorld.__name__ = ["CollectWorld"];
CollectWorld.__super__ = World;
CollectWorld.prototype = $extend(World.prototype,{
	resetCollectables: function() {
		var _g = 0;
		var _g1 = this.collectables;
		while(_g < _g1.length) {
			var collectable = _g1[_g];
			++_g;
			collectable.set_visible(true);
			this.collected = 0;
		}
	}
	,drawBackground: function(numColumns,lineWidth) {
	...
	,update: function(deltaTime) {
		World.prototype.update.call(this,deltaTime);
		this.lineBackground.set_x(this.songObjectXOffset * 1.5);
		while(this.lineBackground.get_x() > this.instance.screenWidth) {
			var _g = this.lineBackground;
			_g.set_x(_g.get_x() - this.columnWidth);
		}
		while(this.lineBackground.get_x() < 0) {
			var _g1 = this.lineBackground;
			_g1.set_x(_g1.get_x() + this.columnWidth);
		}
		var _g2 = this.collectableContainer;
		_g2.set_rotation(_g2.get_rotation() + 3);
		var _g3 = 0;
		var _g11 = this.collectables;
		while(_g3 < _g11.length) {
			var collectable = _g11[_g3];
			++_g3;
			if(this.check(collectable) && collectable.get_visible()) {
				this.collected += 1;
				collectable.set_visible(false);
			}
		}
		var _g4 = 0;
		var _g12 = this.obstacles;
		while(_g4 < _g12.length) {
			var obstacle = _g12[_g4];
			++_g4;
			if(this.check(obstacle)) this.resetCollectables();
		}
		if(this.collected / this.collectables.length > .9) {
			if(!this.playing) {
				this.soundChannel2 = this.song2.play(this.soundChannel.get_position(),-1);
				this.playing = true;
			}
		...
		if(this.check(this.overworldTeleporter) && this.collected / this.collectables.length > .9) {
			this.soundChannel.stop();
			this.soundChannel2.stop();
			this.instance.collectWorld = true;
			this.instance.loadOverworld();
		}
		haxe.Log.trace(this.collected / this.collectables.length,{ fileName : "CollectWorld.hx", lineNumber : 175, className : "CollectWorld", methodName : "update"});
		this.playerCollectable.set_scaleX(this.playerCollectable.set_scaleY(this.collected / this.collectables.length));
	}
	,__class__: CollectWorld
});
...
var DefaultAssetLibrary = function() {
	...
	var id;
	id = "assets/lvl2-add.xmp";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.TEXT);
	id = "assets/lvl3-base.xmp";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.TEXT);
	id = "assets/placeholder.ogg";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.SOUND);
	id = "assets/snare.ogg";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.SOUND);
	id = "assets/lvl1-base.ogg";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.SOUND);
	id = "assets/lvl2-add.ogg";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.SOUND);
	id = "assets/end.png";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.IMAGE);
	id = "assets/lvl3-add.ogg";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.SOUND);
	id = "assets/hat.ogg";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.SOUND);
	id = "assets/lvl1-add.xmp";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.TEXT);
	id = "assets/fart.ogg";
	this.path.set(id,id);
	...
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.TEXT);
	id = "assets/lvl3-base.ogg";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.SOUND);
	id = "assets/lvl1-base.xmp";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.TEXT);
	id = "assets/creditsong.xmp";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.TEXT);
	id = "assets/lvl2-base.ogg";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.SOUND);
	id = "assets/title.png";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.IMAGE);
	id = "assets/lvl2-base.xmp";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.TEXT);
	id = "assets/overworld-base.ogg";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.SOUND);
	id = "assets/creditsong.ogg";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.MUSIC);
	id = "assets/overworld-base.xmp";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.TEXT);
	...
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.TEXT);
	id = "assets/lvl1-add.ogg";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.SOUND);
	id = "assets/lvl3-add.xmp";
	this.path.set(id,id);
	this.type.set(id,openfl.AssetType.TEXT);
};
...
var EndScreen = function(main) {
	this.columnStart = 50;
	this.bpm = 56;
	this.maxColumns = 5;
	this.song = openfl.Assets.getSound("assets/placeholder.ogg");
	this.song2 = openfl.Assets.getSound("assets/placeholder.ogg");
	this.loopBeats = 64;
	World.call(this,main);
	this.start();
	this.playerCollectable.set_visible(false);
	this.addChild(new openfl.display.Bitmap(openfl.Assets.getBitmapData("assets/end.png")));
	openfl.Assets.getSound("assets/creditsong.ogg").play();
};
$hxClasses["EndScreen"] = EndScreen;
EndScreen.__name__ = ["EndScreen"];
EndScreen.__super__ = World;
EndScreen.prototype = $extend(World.prototype,{
	__class__: EndScreen
});
var Hitbox = function(offX,offY,w,h) {
	this.offsetX = offX;
	this.offsetY = offY;
	this.width = w;
	this.height = h;
...
Hitbox.__name__ = ["Hitbox"];
Hitbox.prototype = {
	check: function(xPos,yPos,other,otherX,otherY) {
		if(xPos + this.offsetX >= otherX + other.offsetX + other.width) return false;
		if(xPos + this.offsetX + this.width <= otherX + other.offsetX) return false;
		if(yPos + this.offsetY >= otherY + other.offsetY + other.height) return false;
		if(yPos + this.offsetY + this.height <= otherY + other.offsetY) return false;
		return true;
	}
	,__class__: Hitbox
};
...
IMap.__name__ = ["IMap"];
Math.__name__ = ["Math"];
var MazeWorld = function(main) {
	this.columnStart = 50;
	this.bpm = 112;
	this.maxColumns = 5;
	this.song = openfl.Assets.getSound("assets/lvl3-base.ogg");
	this.song2 = openfl.Assets.getSound("assets/lvl3-add.ogg");
	this.loopBeats = 128;
	World.call(this,main);
	this.drawBackground(this.maxColumns,4);
	this.overworldTeleporterSprite = this.createSprite(14540253,30,this.loopBeats * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4);
	...
	this.collectable = this.spawnObject(12,this.collectableSprite,6,200);
	this.obstacles = new Array();
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,43,240));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,45,180));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,46,40));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,46,80));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,47,340));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,48,240));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,49,300));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,51,110));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,54,90));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,55,120));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,57,380));
	this.obstacles.push(this.spawnObject(20,this.obstacleSprite,57,200));
	this.start();
};
$hxClasses["MazeWorld"] = MazeWorld;
MazeWorld.__name__ = ["MazeWorld"];
MazeWorld.__super__ = World;
MazeWorld.prototype = $extend(World.prototype,{
	drawBackground: function(numColumns,lineWidth) {
		this.columnWidth = this.instance.screenWidth / this.maxColumns;
		this.lineBackground = new openfl.display.Sprite();
		this.lineBackground.get_graphics().beginFill(328967);
...
var Overworld = function(main) {
	this.columnStart = 50;
	this.bpm = 56;
	this.maxColumns = 5;
	this.song = openfl.Assets.getSound("assets/overworld-base.ogg");
	this.song2 = openfl.Assets.getSound("assets/placeholder.ogg");
	this.collectWorldSong = openfl.Assets.getSound("assets/lvl1-add.ogg");
	this.avoidWorldSong = openfl.Assets.getSound("assets/lvl2-add.ogg");
	this.mazeWorldSong = openfl.Assets.getSound("assets/lvl3-add.ogg");
	this.loopBeats = 64;
	World.call(this,main);
	this.collectWorldTeleporterSprite = this.createSprite(1170722,30,this.loopBeats * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4);
	this.avoidWorldTeleporterSprite = this.createSprite(1170722,30,this.loopBeats * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4);
	this.mazeWorldTeleporterSprite = this.createSprite(1170722,30,this.loopBeats * (1 / this.bpm * 60 * 1000) * this.objectBaseSpeed / 4);
	if(!this.instance.collectWorld) this.collectWorldTeleporter = this.spawnObject(30,this.collectWorldTeleporterSprite,10,this.instance.screenWidth / 2 + 100); else this.collectWorldTeleporter = this.spawnObject(30,this.collectWorldTeleporterSprite,40,this.instance.screenWidth / 2 + 50);
	if(!this.instance.avoidWorld) this.avoidWorldTeleporter = this.spawnObject(30,this.avoidWorldTeleporterSprite,20,this.instance.screenWidth / 2 + 100); else this.avoidWorldTeleporter = this.spawnObject(30,this.avoidWorldTeleporterSprite,40,this.instance.screenWidth / 2 - 50);
	if(!this.instance.mazeWorld) this.mazeWorldTeleporter = this.spawnObject(30,this.mazeWorldTeleporterSprite,30,this.instance.screenWidth / 2 + 100); else this.mazeWorldTeleporter = this.spawnObject(30,this.mazeWorldTeleporterSprite,42,this.instance.screenWidth / 2);
	this.instance.addChildAt(this.collectWorldTeleporter,this.instance.getChildIndex(this.player));
	this.instance.addChildAt(this.avoidWorldTeleporter,this.instance.getChildIndex(this.player));
	this.instance.addChildAt(this.mazeWorldTeleporter,this.instance.getChildIndex(this.player));
	this.start();
	this.playerCollectable.set_visible(false);
};
$hxClasses["Overworld"] = Overworld;
Overworld.__name__ = ["Overworld"];
Overworld.__super__ = World;
Overworld.prototype = $extend(World.prototype,{
	start: function() {
		World.prototype.start.call(this);
		if(this.instance.collectWorld) this.soundChannel3 = this.collectWorldSong.play();
		if(this.instance.avoidWorld) this.soundChannel4 = this.avoidWorldSong.play();
		if(this.instance.mazeWorld) this.soundChannel5 = this.mazeWorldSong.play();
	}
	,update: function(deltaTime) {
		World.prototype.update.call(this,deltaTime);
		if(this.check(this.collectWorldTeleporter) && !this.instance.collectWorld) {
			this.soundChannel.stop();
			if(this.soundChannel2 != null) this.soundChannel2.stop();
			if(this.soundChannel3 != null) this.soundChannel3.stop();
			if(this.soundChannel4 != null) this.soundChannel4.stop();
			if(this.soundChannel5 != null) this.soundChannel5.stop();
			this.instance.loadCollectWorld();
		}
		if(this.check(this.avoidWorldTeleporter) && !this.instance.avoidWorld) {
			this.soundChannel.stop();
			if(this.soundChannel2 != null) this.soundChannel2.stop();
			if(this.soundChannel3 != null) this.soundChannel3.stop();
			if(this.soundChannel4 != null) this.soundChannel4.stop();
			if(this.soundChannel5 != null) this.soundChannel5.stop();
			this.instance.loadAvoidWorld();
		}
		if(this.check(this.mazeWorldTeleporter) && !this.instance.mazeWorld) {
			this.soundChannel.stop();
			if(this.soundChannel2 != null) this.soundChannel2.stop();
			if(this.soundChannel3 != null) this.soundChannel3.stop();
			if(this.soundChannel4 != null) this.soundChannel4.stop();
			if(this.soundChannel5 != null) this.soundChannel5.stop();
			this.instance.loadMazeWorld();
		}
	}
	,__class__: Overworld
});
...
var SongObject = function(newSprite,radius,yStart,xStart,screenWidth,yRespawnDistance,ySpeed,objectType) {
	...
	this.startX = this.set_x(xStart);
	this.startY = this.set_y(yStart);
	this.yVelocity = ySpeed;
	this.inColumn = true;
};
$hxClasses["SongObject"] = SongObject;
SongObject.__name__ = ["SongObject"];
SongObject.viewport = null;
...
SongObject.prototype = $extend(openfl.display.Sprite.prototype,{
	...
	,update: function(time,xOffset) {
		if(xOffset == null) xOffset = 0;
		if(time == null) time = 0;
		if(this.yVelocity > 0) this.set_y(this.startY + time * this.yVelocity);
		this.set_x(this.startX + xOffset);
		while(this.get_x() > this.screenWidth) {
			var _g = this;
			_g.set_x(_g.get_x() - this.screenWidth);
		}
		while(this.get_x() < 0) {
			var _g1 = this;
			_g1.set_x(_g1.get_x() + this.screenWidth);
		}
		var tileData = new Array();
		tileData = [];
		var _g2 = 0;
		var _g11 = [-this.screenWidth,0,this.screenWidth];
		while(_g2 < _g11.length) {
			var drawX = _g11[_g2];
			++_g2;
			var _g21 = 0;
			var _g3 = [-this.yRespawnDistance,0,this.yRespawnDistance];
			while(_g21 < _g3.length) {
				var drawY = _g3[_g21];
				++_g21;
				tileData.push(this.get_x() + drawX - this.size);
				tileData.push(this.get_y() + drawY - this.size);
				tileData.push(0);
			}
		}
		if(this.get_visible()) this.sprite.drawTiles(SongObject.viewport.get_graphics(),tileData);
	}
...
var TitleScreen = function(main) {
	...
	this.start();
	this.playerCollectable.set_visible(false);
	this.addChild(new openfl.display.Bitmap(openfl.Assets.getBitmapData("assets/title.png")));
};
$hxClasses["TitleScreen"] = TitleScreen;
TitleScreen.__name__ = ["TitleScreen"];
TitleScreen.__super__ = World;
TitleScreen.prototype = $extend(World.prototype,{
	update: function(deltaTime) {
		World.prototype.update.call(this,deltaTime);
		if(haxel.KeyboardInput.check(37) || haxel.KeyboardInput.check(39) || haxel.MouseInput.mouseDown) {
			this.soundChannel.stop();
			if(this.soundChannel2 != null) this.soundChannel2.stop();
			this.instance.loadOverworld();
		}
	}
	,__class__: TitleScreen
});
var Type = function() { };
$hxClasses["Type"] = Type;
Type.__name__ = ["Type"];
...
haxel.Key.init = function() {
};
haxel.Key.keyCode = function(e) {
	return e.keyCode;
};
haxel.Key.nameOfKey = function($char) {
	if($char == -1) return "";
...
haxel.KeyboardInput = function() { };
$hxClasses["haxel.KeyboardInput"] = haxel.KeyboardInput;
haxel.KeyboardInput.__name__ = ["haxel","KeyboardInput"];
haxel.KeyboardInput.lastKey = null;
haxel.KeyboardInput.check = function(input) {
	if(input < 0) return haxel.KeyboardInput._keyNum > 0; else return haxel.KeyboardInput._key[input];
};
haxel.KeyboardInput.pressed = function(input) {
	if(input < 0) return haxel.KeyboardInput._pressNum != 0; else return haxel.KeyboardInput.indexOf(haxel.KeyboardInput._press,input) >= 0;
};
haxel.KeyboardInput.released = function(input) {
	if(input < 0) return haxel.KeyboardInput._releaseNum != 0; else return haxel.KeyboardInput.indexOf(haxel.KeyboardInput._release,input) >= 0;
};
haxel.KeyboardInput.init = function() {
	if(!haxel.KeyboardInput._initialized) {
		haxel.Core.instance.stage.addEventListener(openfl.events.KeyboardEvent.KEY_DOWN,haxel.KeyboardInput.onKeyDown,false,2);
		haxel.Core.instance.stage.addEventListener(openfl.events.KeyboardEvent.KEY_UP,haxel.KeyboardInput.onKeyUp,false,2);
		haxel.Key.init();
		haxel.KeyboardInput._initialized = true;
	}
};
haxel.KeyboardInput.update = function() {
	while(haxel.KeyboardInput._pressNum-- > -1) haxel.KeyboardInput._press[haxel.KeyboardInput._pressNum] = -1;
	haxel.KeyboardInput._pressNum = 0;
	while(haxel.KeyboardInput._releaseNum-- > -1) haxel.KeyboardInput._release[haxel.KeyboardInput._releaseNum] = -1;
	haxel.KeyboardInput._releaseNum = 0;
};
haxel.KeyboardInput.onKeyDown = function(e) {
	var code = haxel.Key.keyCode(e);
	if(code == -1) return;
	haxel.KeyboardInput.lastKey = code;
	if(code == 8) haxel.KeyboardInput.keyString = HxOverrides.substr(haxel.KeyboardInput.keyString,0,haxel.KeyboardInput.keyString.length - 1); else if(code > 47 && code < 58 || code > 64 && code < 91 || code == 32) {
		if(haxel.KeyboardInput.keyString.length > 100) haxel.KeyboardInput.keyString = HxOverrides.substr(haxel.KeyboardInput.keyString,1,null);
		var $char = String.fromCharCode(code);
		if(e.shiftKey != haxel.KeyboardInput.check(20)) $char = $char.toUpperCase(); else $char = $char.toLowerCase();
		haxel.KeyboardInput.keyString += $char;
	}
	if(!haxel.KeyboardInput._key[code]) {
		haxel.KeyboardInput._key[code] = true;
		haxel.KeyboardInput._keyNum++;
		haxel.KeyboardInput._press[haxel.KeyboardInput._pressNum++] = code;
	}
};
haxel.KeyboardInput.onKeyUp = function(e) {
	var code = haxel.Key.keyCode(e);
	if(code == -1) return;
	if(haxel.KeyboardInput._key[code]) {
		haxel.KeyboardInput._key[code] = false;
		haxel.KeyboardInput._keyNum--;
		haxel.KeyboardInput._release[haxel.KeyboardInput._releaseNum++] = code;
	}
};
haxel.KeyboardInput.indexOf = function(a,v) {
	var i = 0;
	var _g = 0;
	while(_g < a.length) {
		var v2 = a[_g];
...
haxel.MouseInput = function() { };
$hxClasses["haxel.MouseInput"] = haxel.MouseInput;
haxel.MouseInput.__name__ = ["haxel","MouseInput"];
haxel.MouseInput.mouseDown = null;
haxel.MouseInput.mouseUp = null;
haxel.MouseInput.mousePressed = null;
haxel.MouseInput.mouseReleased = null;
haxel.MouseInput.mouseWheel = null;
haxel.MouseInput.get_mouseWheelDelta = function() {
	if(haxel.MouseInput.mouseWheel) {
		haxel.MouseInput.mouseWheel = false;
		return haxel.MouseInput._mouseWheelDelta;
	}
	return 0;
};
...
haxel.MouseInput.init = function() {
	if(!haxel.MouseInput._initialized) {
		haxel.Core.instance.stage.addEventListener(openfl.events.MouseEvent.MOUSE_DOWN,haxel.MouseInput.onMouseDown,false,2);
		haxel.Core.instance.stage.addEventListener(openfl.events.MouseEvent.MOUSE_UP,haxel.MouseInput.onMouseUp,false,2);
		haxel.Core.instance.stage.addEventListener(openfl.events.MouseEvent.MOUSE_WHEEL,haxel.MouseInput.onMouseWheel,false,2);
		haxel.MouseInput._initialized = true;
	}
};
haxel.MouseInput.update = function() {
	if(haxel.MouseInput.mousePressed) haxel.MouseInput.mousePressed = false;
	if(haxel.MouseInput.mouseReleased) haxel.MouseInput.mouseReleased = false;
};
haxel.MouseInput.onMouseDown = function(e) {
	if(!haxel.MouseInput.mouseDown) {
		haxel.MouseInput.mouseDown = true;
		haxel.MouseInput.mouseUp = false;
		haxel.MouseInput.mousePressed = true;
	}
};
haxel.MouseInput.onMouseUp = function(e) {
	haxel.MouseInput.mouseDown = false;
	haxel.MouseInput.mouseUp = true;
	haxel.MouseInput.mouseReleased = true;
};
haxel.MouseInput.onMouseWheel = function(e) {
	haxel.MouseInput.mouseWheel = true;
	haxel.MouseInput._mouseWheelDelta = e.delta;
};
haxel.Time = function() { };
$hxClasses["haxel.Time"] = haxel.Time;
haxel.Time.__name__ = ["haxel","Time"];
haxel.Time.callbackFunction = null;
haxel.Time.postCallbackFunction = null;
haxel.Time.rate = null;
haxel.Time.get_rate = function() {
	return 1000 / haxel.Time.framerate;
};
haxel.Time.skip = null;
haxel.Time.get_skip = function() {
	return haxel.Time.get_rate() * (haxel.Time.maxFrameSkip + 1);
};
haxel.Time.last = null;
haxel.Time.timer = null;
haxel.Time.init = function() {
	haxel.Time.last = openfl.Lib.getTimer();
	haxel.Time.timer = new haxe.Timer(haxel.Time.tickRate);
	haxel.Time.timer.run = haxel.Time.onTimerTick;
};
haxel.Time.onTimerTick = function() {
	var time = openfl.Lib.getTimer();
	haxel.Time.delta += time - haxel.Time.last;
	haxel.Time.last = time;
	if(haxel.Time.delta < haxel.Time.get_rate()) return;
	var framesCalled = 0;
	while(haxel.Time.delta >= haxel.Time.get_rate()) {
		if(haxel.Time.callbackFunction != null) {
			if(!haxel.Time.paused) haxel.Time.callbackFunction(haxel.Time.get_rate());
		}
		if(!haxel.Time.paused) haxel.Core.updateFrame();
		haxel.Time.delta -= haxel.Time.get_rate();
		framesCalled += 1;
	}
	if(haxel.Time.postCallbackFunction != null) {
		if(!haxel.Time.paused) haxel.Time.postCallbackFunction(framesCalled);
	}
	if(!haxel.Time.paused) haxel.Core.updatePostFrame();
};
var js = {};
js.Boot = function() { };
...
haxel.Key.DOWN = 40;
haxel.Key.ENTER = 13;
haxel.Key.COMMAND = 15;
haxel.Key.CONTROL = 17;
haxel.Key.SPACE = 32;
haxel.Key.SHIFT = 16;
haxel.Key.BACKSPACE = 8;
haxel.Key.CAPS_LOCK = 20;
haxel.Key.DELETE = 46;
haxel.Key.END = 35;
haxel.Key.ESCAPE = 27;
haxel.Key.HOME = 36;
haxel.Key.INSERT = 45;
haxel.Key.TAB = 9;
haxel.Key.PAGE_DOWN = 34;
haxel.Key.PAGE_UP = 33;
haxel.Key.LEFT_SQUARE_BRACKET = 219;
haxel.Key.RIGHT_SQUARE_BRACKET = 221;
haxel.Key.TILDE = 192;
haxel.Key.A = 65;
haxel.Key.B = 66;
haxel.Key.C = 67;
...
haxel.Key.F14 = 125;
haxel.Key.F15 = 126;
haxel.Key.DIGIT_0 = 48;
haxel.Key.DIGIT_1 = 49;
haxel.Key.DIGIT_2 = 50;
haxel.Key.DIGIT_3 = 51;
haxel.Key.DIGIT_4 = 52;
haxel.Key.DIGIT_5 = 53;
haxel.Key.DIGIT_6 = 54;
haxel.Key.DIGIT_7 = 55;
haxel.Key.DIGIT_8 = 56;
haxel.Key.DIGIT_9 = 57;
haxel.Key.NUMPAD_0 = 96;
haxel.Key.NUMPAD_1 = 97;
haxel.Key.NUMPAD_2 = 98;
haxel.Key.NUMPAD_3 = 99;
haxel.Key.NUMPAD_4 = 100;
haxel.Key.NUMPAD_5 = 101;
haxel.Key.NUMPAD_6 = 102;
haxel.Key.NUMPAD_7 = 103;
haxel.Key.NUMPAD_8 = 104;
haxel.Key.NUMPAD_9 = 105;
haxel.Key.NUMPAD_ADD = 107;
haxel.Key.NUMPAD_DECIMAL = 110;
haxel.Key.NUMPAD_DIVIDE = 111;
haxel.Key.NUMPAD_ENTER = 108;
haxel.Key.NUMPAD_MULTIPLY = 106;
haxel.Key.NUMPAD_SUBTRACT = 109;
haxel.Key._nativeCorrection = new haxe.ds.StringMap();
haxel.KeyboardInput.keyString = "";
haxel.KeyboardInput.kKeyStringMax = 100;
haxel.KeyboardInput._initialized = false;
haxel.KeyboardInput._key = new Array();
haxel.KeyboardInput._keyNum = 0;
haxel.KeyboardInput._press = new Array();
haxel.KeyboardInput._pressNum = 0;
haxel.KeyboardInput._release = new Array();
haxel.KeyboardInput._releaseNum = 0;
haxel.MouseInput._initialized = false;
haxel.MouseInput._mouseWheelDelta = 0;
haxel.Time.framerate = 30;
haxel.Time.maxElapsed = 0.0333;
